...
def construct_hyper_param():
    ...
    parser.add_argument("--target_dir",
                        default='/opt/disk1/image/全球视频云创新挑战赛/copy_paste',
                        type=str,
                        help="target_dir")
...
def main():
    ...
    video_idx_list2 = os.listdir(os.path.join(args.src_dir, 'JPEGImages'))
    random.shuffle(video_idx_list1)
    random.shuffle(video_idx_list2)

    videl_id_pairs = []
    for video_idx1 in video_idx_list1:
        if len(videl_id_pairs) >= total:
            break

        for video_idx2 in random.sample(video_idx_list2, 3):
            if video_idx1 != video_idx2:
                videl_id_pairs.append((video_idx1, video_idx2))
                if len(videl_id_pairs) >= total:
                    break

    for ii, (video_idx1, video_idx2) in tqdm(enumerate(videl_id_pairs), total=len(videl_id_pairs)):
